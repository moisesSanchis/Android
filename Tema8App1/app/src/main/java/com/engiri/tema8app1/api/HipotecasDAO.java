...
public class HipotecasDAO {
    ...
    public long update(ContentValues reg) {
        ...
        if (reg.containsKey(C_COLUMNA_ID)) {
            // Obtenemos el id y lo borramos de los valores a actualizar,
            // ya que el id no se actualizar
            long id = reg.getAsLong(C_COLUMNA_ID);
            reg.remove(C_COLUMNA_ID);

            // Actualizamos el registro con el identificador que hemos extraído
